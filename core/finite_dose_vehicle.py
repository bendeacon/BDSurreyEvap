...
class Vehicle:
    ...
    def dried_vehicle(self, Volume, A, y, Temp, t, args, h):
        ...
        if phase == "SOLID":
            ...
            P_Solute =  14 #Chemical("Acetophenone").VaporPressure.solve_property(1e5)  #0.065 #Vapor pressure of Active Pascal
    ...
    def Evaporation_Solution(self, Volume, A, y, Temp, x1, x0, Name_Solute, flux, h):
        ...
        r_Solute = ( 0.9087 * self.chem.mw * 3/4/np.pi ) ** (1.0/3) * 1e-10
        P_Solute = 14 #Chemical("Nitrobenzene").VaporPressure.solve_property(1e5) #0.065 # #Vapor pressure of Active Pascal
    ...
    def Evaporation_Precipitate(self, V, A, y, Temp, x1, x0, Name_Solute, flux, h, V1, V2):
        ...
        Volume_Active = Volume_Active * (1e+21) #volume in nm3/particle
        r_Solute = ( 0.9087 * self.chem.mw * 3/4/np.pi ) ** (1.0/3) * 1e-10
        P_Solute = 14 #Chemical("Acetophenone").VaporPressure.solve_property(1e5)  #Vapor pressure of Active Pascal
        D_Solute =  scipy.constants.Boltzmann*Temp/(6*math.pi*0.0000000917*r_Solute)#0 #Diffusion coefficient of Active in air m2/s   
